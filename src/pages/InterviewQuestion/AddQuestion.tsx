...
export default function AddQuestion({ observation, onClick }: any) {
  ...
  const handleOnClick = (event: any) => {
    if (event.target.id === "container" || event.target.id === "Submit")
      onClick();
  };
  ...
  const handleSubmitAdd = (e: any) => {
    ...
    if (data.typeQuestion === "") {
      toast.promise(InterviewService.error(data.typeQuestion), {
        error: "Please select Type ",
      });
    }
    if (data.skillId === null) {
      toast.promise(InterviewService.error(data.skillId), {
  ...
  return (
    ...
    >
      <div className="relative flex flex-col w-1/2 bg-white rounded-lg h-3/4 drop-shadow-md">
        <div className="flex justify-start m-5 text-lg font-medium">
          Add Question
        </div>
