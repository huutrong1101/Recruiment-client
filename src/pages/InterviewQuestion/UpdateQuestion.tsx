...
export default function UpdateQuestion({
  ...
  useEffect(() => {
    const getQuestionDetail = async () => {
      const response = await axiosInstance.get(
        `interviewer/question/${questionID}`,
      );
  ...
  const handleOnClick = (event: any) => {
    if (event.target.id === "container" || event.target.id === "Update")
      onClick();
  };
  ...
  const skills: QuestionListInterface[] = useAppSelector(
    (state) => state.questionList.skills,
  );
  const types: QuestionListInterface[] = useAppSelector(
    (state) => state.questionList.types,
  );
  ...
  const handleSubmitUpdate = (e: any) => {
    e.preventDefault();
    const foundSkill: any = showSkills.find(
      (skill) => skill.name === inputData.skill,
    );
    const data = {
  ...
  return (
    <div
      className="fixed inset-0 bg-black bg-opacity-10 backdrop-blur-sm flex justify-center items-center rounded-lg z-20 "
      id="Update"
        ...
        <div className="flex h-full">
          <div className="flex flex-col w-3/5 h-[83%] ">
            ...
            <div className="w-full h-full flex justify-center">
              <div className=" w-11/12 h-full border-2 border-emerald-600 rounded-md ml-6 p-2">
                <textarea
                  className="w-full  resize-none outline-none h-full"
                  placeholder="Answer"
          ...
          <div className="flex flex-col w-2/5 p-2 relative">
                ...
                <Menu as="div" className=" h-fit">
                  <div className="absolute w-2/3  ">
                    <div className="w-full h-full  ">
                      <Menu.Button
                        className="w-full p-3  bg-emerald-600 rounded-md text-white border border-transparent
            ...
            <Menu as="div" className="w-full h-[25%] relative flex flex-col">
              <div className="absolute w-2/3  ">
                <Menu.Button
                  className="w-full h-fit p-3 my-1 bg-emerald-600 rounded-md text-white border border-transparent
                                active:border-emerald-600  active:text-emerald-600 
                                 active:bg-white flex items-center"
                  onClick={handleActive}
                >
            ...
            <div className="w-full h-1/5 font-normal text-md my-6  px-3">
              <div className="w-full h-full flex justify-end items-end ">
                <button
                  className=" w-fit h-fit px-6 py-2 bg-emerald-600 rounded-md text-white border border-transparent
