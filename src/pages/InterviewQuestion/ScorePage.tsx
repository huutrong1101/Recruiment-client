...
export default function ScorePage() {
   ...
   const question: QuestionListInterface[] = useAppSelector((state) => state.questionList.questionList,);
                  ...
                  <div className="flex justify-center my-4 ">
                     <div className=" rounded-lg border-gray-200 border-2 w-11/12 h-fit">
                           ...
                           <table className="w-full  ">
                              <thead className="w-full ">
                                 <tr className="flex justify-between  mt-3 ml-4 ">
                                    ...
                                    <th className="text-lg tracking-wide text-left font-semibold basis-3/6  ">Question</th>
                                    <th className="text-lg tracking-wide text-left font-semibold basis-1/6  ">Note</th>
                                 </tr>
                              </thead>
                              <tbody className="">
                                 <div className="grid text-left min-h-[30vh] max-h-[30vh]">
                                    <div>
                                       {showQuestion.map((question: any) => (
                                          ...
                                          border-2 border-white hover: hover:border-emerald-600 hover:rounded-lg hover:text-black hover:transition-all duration-100"
                                             ...
                                             <td className="basis-1/6" onClick={() => handleQuestionClick(question.content)}>{question.skill}</td>
                                             <td className="basis-1/6" onClick={() => handleQuestionClick(question.content)}>{question.typeQuestion}</td>
                                             <td className="basis-3/6 flex-nowrap" onClick={() => handleQuestionClick(question.content)}>{question.content}</td>
                                             <td className="basis-1/6 flex-nowrap" onClick={() => handleQuestionClick(question.content)}>{question.note}</td>
                                          </tr>
                                       ))}
                                    </div>
                                 </div>
                        ...
                        <div className="flex justify-end m-4">
                           <Pagination queryConfig={queryConfig} pageSize={pageSize} url="/interviewer/score-page" />
                        </div>
                     </div>
                  </div>
                  {/* score */}
                  <div className="flex flex-col gap-y-3 place-items-center" >
                     <div className="flex flex-col  w-11/12  ">
                        <div className="bg-emerald-600 h-fit w-full rounded-lg p-2 pb-2 relative 
                                       drop-shadow-lg hover:bg-emerald-700 hover:transition-all mb-3">
                           <div className="flex flex-col">
                              <div className=" inline-flex gap-3 w-full h-full ">
                                 <div className=" text-white font-semibold flex flex-col w-full justify-start">Question
                                    <div className="w-full h-fit bg-white rounded-md flex items-center p-2 ">
                        ...
                        <div className=" h-fit flex flex-row w-full justify-between mb-3">
                           {/* Prev - Next */}
