...
export default function PrintResume() {
  ...
  useEffect(() => {
    ...
    UserService.getUserInformation()
      ...
      .catch(() =>
        toast.error(`There was an error when fetching user information`),
      )
      .finally(() => {
        setLoading(false);
      });
  ...
  return (
    <div className={`print-resume`}>
      ...
      <div className="inset-0 overflow-y-auto border mb-4 print:border-0 print:mb-0">
        <div className="flex flex-col p-6 print:p-2">
          ...
          {loading ? (
            <div
              className={classNames(
                `flex flex-col items-center justify-center text-3xl min-h-[50vh]`,
              )}
            >
              <LoadSpinner />
            </div>
            ...
            <div>
              ...
              <div className={classNames(`flex flex-col flex-wrap max-h-80vh`)}>
                {[...Object.keys(data)].map((keyId, _idx) => {
                  ...
                  return (
                    <div
                      className={classNames(`text-zinc-500  mb-12`)}
                      key={keyId}
                    >
                      <h1
                        className={classNames(
                          `font-semibold text-emerald-800 text-2xl capitalize`,
                        )}
                      >
                        {keyId}
                      </h1>
                        ...
                        {keyId !== "skills" ? (
                          ...
                          <>
                            ...
                            {data[keyId].map(({ label, _value }, _index) => (
                              <span
                                className={classNames(
                                  `text-emerald-600 px-4 py-4 inline-block`,
                                )}
                              >
                                {label}
                              </span>
