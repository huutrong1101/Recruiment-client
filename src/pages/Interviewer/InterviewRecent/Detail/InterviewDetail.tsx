...
const InterviewDetail = () => {
  ...
  const {
    searchQuestions,
    searchQuestionsStatus,
    selectedQuestions,
    assignedQuestions,
    assignedQuestionsStatus,
  ...
  const handleChange = (event: any) => {
    ...
    dispatch(
      fetchINTQuestionData(
        `?content=${textFilter}&skill=${skill}&typeQuestion=${type}`,
      ),
    );
    setTimeout(() => {
  ...
  const handleClickType = (event: any) => {
    ...
    dispatch(
      fetchINTQuestionData(
        `?content=${text}&skill=${skill}&typeQuestion=${typeFilter}`,
      ),
    );
    setTimeout(() => {
      setShowDropdown(true);
  ...
  const handleClickSkill = (event: any) => {
    ...
    dispatch(
      fetchINTQuestionData(
        `?content=${text}&skill=${skillFilter}&typeQuestion=${type}`,
      ),
    );
    setTimeout(() => {
      setShowDropdown(true);
  ...
  if (
      ...
      <div className="InterviewDetail ">
        <div className="mt-8 border-2 shadow-xl px-6 py-6 rounded-xl">
          ...
          <div className="text-base mt-2">
            Job Name:{" "}
            <span className="text-sm ml-2">{INTSingleInterview?.jobName}</span>
          </div>
          <div className="text-base">
            Position Recruiment:{" "}
            <span className="text-sm ml-2">
              {JOB_POSITION[INTSingleInterview?.position]}
            </span>
          </div>
          <div className="text-base">
            Date:{" "}
            <span className="text-sm ml-2">
              {formatHHMM(INTSingleInterview?.time) +
                " " +
                formatDDMMYY(INTSingleInterview?.time)}
            </span>
          </div>
          <div className="text-base">
            Link Meeting:
            <Link to={INTSingleInterview?.interviewLink}>
              <span
                className="text-sm ml-2 text-blue-600 hover:text-blue-900 
        ...
        {!checkCompleteMarkScore(assignedQuestions) && (
          <div className="mt-14 border-2 shadow-xl px-6 py-6 rounded-xl">
            <div className="text-2xl font-semibold">
              Assign Questions For Interview
            </div>
                          ...
                          <td className="border px-4 py-2 text-center">
                            <div className="flex justify-center">
                              <TrashIcon
                                ...
                                className="ml-3 cursor-pointer w-5 h-5 text-gray-500 mr-2"
                              />
            ...
            <div className="mt-14 border-2 shadow-xl px-6 py-6 rounded-xl">
              ...
              <div className="flex justify-end text-base mt-4">
                ...
                <div className="mr-8">
                  ...
                  <span className="text-xl font-medium">
                    {" "}
                    {calculateTotalScore(assignedQuestions)}/
                    {10 * assignedQuestions.length}
                  </span>
                </div>
                <div>
                  Score out of 100:
                  <span className="text-xl font-medium">
                    {" "}
                    {Math.round(
                      (calculateTotalScore(assignedQuestions) * 1.0 * 100) /
                        (10 * assignedQuestions.length),
                    )}
                  </span>
                </div>
        ...
        {view ? (
          ...
          <div className="flex justify-between mt-8">
            ...
            {!checkCompleteMarkScore(assignedQuestions) &&
              isDateReached(INTSingleInterview?.time) && (
    ...
    INTSingleInterviewStatus === STATUS.ERROR500 ||
    assignedQuestionsStatus === STATUS.ERROR500
  ) {
